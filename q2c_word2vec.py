...
def naive_softmax_loss_and_gradient(
    ...
    # Compute the scores for all words in the vocabulary
    scores = np.dot(center_word_vec, outside_vectors.T)

    # Compute the probabilities using softmax
    exp_scores = np.exp(scores)
    probs = exp_scores / np.sum(exp_scores)

    # Compute the naive softmax loss
    loss = -np.log(probs[outside_word_idx])

    # Compute the gradient with respect to the center word vector
    grad_center_vec = -outside_vectors[outside_word_idx] + np.dot(probs, outside_vectors)
...
def neg_sampling_loss_and_gradient(
    ...
    for idx in indices:
        ...
        outside_word_vec = outside_vectors[idx]

        negative_word_vec = np.dot(center_word_vec, outside_word_vec)

        sigmoid = 1 / (1 + np.exp(-negative_word_vec))

        loss += - target * np.log(sigmoid) - (1 - target) * np.log(1 - sigmoid)

        grad_negative_word_vec = sigmoid - target

        grad_center_vec += grad_negative_word_vec * outside_word_vec

        grad_outside_vectors[idx] += grad_negative_word_vec * center_word_vec

    return loss, grad_center_vec, grad_outside_vectors
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    for outside_word in outside_words:
        ...
        current_loss, current_center_grad, current_outside_grad = word2vec_loss_and_gradient(
            center_word_vectors[center_word_index], outside_index, outside_vectors, dataset)
        loss += current_loss
        grad_center_vecs[center_word_index] += current_center_grad
        grad_outside_vectors += current_outside_grad
    ### END YOUR CODE
...
def test_word2vec_basic():
    ...
    print(skipgram("c", ["a", "b", "e", "d", "b", "c"], dummy_tokens,
